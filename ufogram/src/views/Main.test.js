...
test('Tests My Profile button', async () => {
    act(() => {
        render(
            <MemoryRouter initialEntries={['/main']}> {/* Set the initial route */}
              <Main />
            </MemoryRouter>
          );
    })
    ...
    await waitFor(() => {
        expect(mockedNavigate).toHaveBeenCalledWith('/userprofile', { state: { userId: 2, username: 'lionelhu', self: true, sName:  'lionelhu', sId: 2, users: users, followed: true } });
    });
...
test('Tests new post button', async () => {
    ...
    // expect(newPageElement).toBeInTheDocument();
    await waitFor(() => {
        expect(mockedNavigate).toHaveBeenCalledWith('/newpost', { state: { userId: 2, username: 'lionelhu', users: users } });
    });
    // await waitFor(() => expect(window.location.href).toContain('/signup'));
